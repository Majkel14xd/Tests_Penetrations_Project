...
def get_pracownicy(search_query=None, per_page=8, page=1):
    ...
    if search_query:
        ...
        query = "SELECT * FROM pracownicy WHERE imie LIKE ? OR nazwisko LIKE ? LIMIT ? OFFSET ?"
        cursor.execute(query, (f"%{sanitized_query}%", f"%{sanitized_query}%", per_page, offset))
...
def add_pracownik(imie, nazwisko, stanowisko):
    ...
    cursor.execute(query, (sanitized_imie, sanitized_nazwisko, sanitized_stanowisko))
    conn.commit()
    conn.close()
