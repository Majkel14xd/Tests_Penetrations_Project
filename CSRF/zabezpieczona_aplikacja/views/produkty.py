...
def get_produkty(search_query=None, per_page=8, page=1):
    ...
    if search_query:
        ...
        query = "SELECT * FROM produkty WHERE nazwa LIKE ? LIMIT ? OFFSET ?"
        cursor.execute(query, (f"%{sanitized_query}%", per_page, offset))
...
def add_produkt(nazwa, cena):
    ...
    cursor.execute(query, (sanitized_nazwa, cena))
    conn.commit()
    conn.close()
